...
var preHash=0, myHash=0;

function countOccupiedSeats(posX, posY) {
  // test 8 surrounding seats.
  var count = 0;
  var tX, tY;
  tY = posY - 1;
  if (tY > -1) {
    ...
    // NW
    tX = posX - 1;
    if (tX > -1) count += (line[tX] == '#' ? 1 : 0);
    // N
    tX = posX;
    count += (line[tX] == '#' ? 1 : 0);
    // NE
    tX = posX + 1;
    if (tX < w) count += (line[tX] == '#' ? 1 : 0);
  }
...
function test(posX, posY) {
  ...
  if (seat == '.') return -1;
  // L or #
  var occup = countOccupiedSeats(posX, posY);
  return occup;
}
  ...
  for (i = 0; i < j; i++) {
    ...
    for (var x = 0; x < w; x++) {
      ...
      if (rslt > -1) {
        ...
        // rule 1: # + 4 adjacent seats or more = L
        if (seat == '#' && rslt > 3) {
          line[x] = 'L';
        }
      }
